...
chronology_simulation = function(object, nsims=10000, method="inla",store.means=FALSE,print.progress=FALSE){
  time.start=Sys.time()
  ...
  ## sample mean vector and chronologies
  ...
  if(tolower(method) == "inla"){
    ...
    for(i in 1:nsims){
      ...
      if(store.means) object$simulation$dmean[,i] = dmeansim
      ...
      object$simulation$age[,i] = cumsum(c(object$preceeding$y0,dmeansim+noisesim))[2:(n+1)]
